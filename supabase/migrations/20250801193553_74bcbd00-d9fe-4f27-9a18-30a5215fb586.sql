...
WITH CHECK (
  EXISTS (
    ...
    WHERE access_code::text = current_setting('app.current_access_code', true)
    AND role = 'community_admin'
    AND (destination_id IS NULL OR destination_id = NEW.destination_id)
  )
);

-- 确保钱包地址的权限正确
DROP POLICY IF EXISTS "Community admins manage wallet addresses" ON wallet_addresses;
...
USING (
  EXISTS (
    ...
    LEFT JOIN vehicles v ON v.id = wallet_addresses.vehicle_id
    LEFT JOIN fixed_routes fr ON fr.id = wallet_addresses.route_id
    WHERE u.access_code::text = current_setting('app.current_access_code', true)
    AND u.role = 'community_admin'
    AND (
      u.destination_id = wallet_addresses.destination_id OR
      u.destination_id = v.destination_id OR
      u.destination_id = fr.destination_id
    )
  ) OR is_super_admin()
);
...
USING (
  EXISTS (
    ...
    WHERE access_code::text = current_setting('app.current_access_code', true)
    AND role = 'community_admin'
    AND (destination_id IS NULL OR destination_id = fixed_routes.destination_id)
  ) OR is_super_admin()
);